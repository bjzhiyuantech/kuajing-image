...
export const users = mysqlTable(
  "users",
  {
    ...
    accountStatus: shortText("account_status", 32).notNull().default("active"),
    deletedAt: isoDate("deleted_at"),
    deletionLockUntil: isoDate("deletion_lock_until"),
    planId: id("plan_id"),
...
export const subscriptionPlans = mysqlTable(
  "subscription_plans",
  {
    ...
    currency: shortText("currency", 16).notNull(),
    appleProductId: shortText("apple_product_id", 255),
    appleIapEnabled: int("apple_iap_enabled").notNull(),
    enabled: int("enabled").notNull(),
    sortOrder: int("sort_order").notNull(),
